...
const AddAccountSheet = forwardRef<AddAccountSheetHandle, Props>(({ onAccountSaved }, ref) => {
  ...
  const update = (patch: Partial<ReturnType<typeof initForm>>) => setForm((f) => ({ ...f, ...patch }));
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={[styles.headerTitle, { color: theme.fg }]}>
            {form.step === 1 ? "Add Account" : form.step === 2 ? "Account Details" : "First Entry"}
          </Text>
          <View style={styles.headerBtn} />
        </View>
          ...
          {([1, 2, 3] as Step[]).map((s) => (
            <View
              ...
              style={[
                styles.stepDot,
                {
                  backgroundColor: s <= form.step ? (typeConfig?.color ?? theme.fg) : theme.border,
          ...
          <View style={styles.typeGrid}>
            {TYPE_ORDER.map((type) => {
              ...
              return (
                ...
                >
                  <View style={[styles.typeIconWrap, { backgroundColor: `${cfg.color}20` }]}>
                    <Ionicons name={cfg.glyph as any} size={22} color={cfg.color} />
                  </View>
                  <Text style={[styles.typeName, { color: theme.fg }]}>{cfg.label}</Text>
                  <Text style={[styles.typeDesc, { color: theme.fg3 }]} numberOfLines={2}>
                    {cfg.description}
                  </Text>
                </Pressable>
              );
        ...
        {form.step === 2 && form.type && (
          <View style={styles.form}>
            ...
            >
              <Ionicons name={typeConfig!.glyph as any} size={16} color={typeConfig!.color} />
              <Text style={[styles.selectedTypeLabel, { color: typeConfig!.color }]}>{typeConfig!.label}</Text>
            </View>
            ...
            {form.type === "house" && (
              <>
                ...
                <Field
                  ...
                  placeholder="0"
                />
                <View style={[styles.toggleRow, { backgroundColor: theme.surface2, borderColor: theme.border }]}>
                  <View style={styles.toggleLeft}>
                    <Text style={[styles.toggleLabel, { color: theme.fg }]}>Shared Asset</Text>
                    <Text style={[styles.toggleHint, { color: theme.fg3 }]}>Co-owned property</Text>
                  </View>
                  <Switch
                    value={form.isShared}
        ...
        {form.step === 3 && form.type && (
          <View style={styles.form}>
            <DateSelector label="Date" value={form.date} onChange={(d) => update({ date: d })} />
            ...
            <Field
              ...
              autoFocus
              hint={
                form.type === "credit_card" || form.type === "loan" ? "Enter the amount you owe (positive)" : undefined
              }
            />
